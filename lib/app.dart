...
class _TolongTukarAppState extends State<TolongTukarApp> {
  ...
  ThemeData _theme(bool dark){const ink=Color(0xFF111111),yellow=Color(0xFFFFD02B),cream=Color(0xFFF6F3E6),teal=Color(0xFF7AD9CC);final scheme=ColorScheme.fromSeed(seedColor:yellow,brightness:dark?Brightness.dark:Brightness.light,surface:dark?const Color(0xFF202020):Colors.white);return ThemeData(colorScheme:scheme,scaffoldBackgroundColor:dark?const Color(0xFF161616):cream,useMaterial3:true,fontFamily:'sans-serif',cardTheme:CardThemeData(shape:RoundedRectangleBorder(side:BorderSide(color:dark?Colors.white:ink,width:2),borderRadius:BorderRadius.circular(4)),elevation:0),inputDecorationTheme:InputDecorationTheme(filled:true,fillColor:scheme.surface,border:const OutlineInputBorder(borderSide:BorderSide(color:ink,width:2),borderRadius:BorderRadius.zero)),filledButtonTheme:FilledButtonThemeData(style:FilledButton.styleFrom(backgroundColor:yellow,foregroundColor:ink,shape:const RoundedRectangleBorder(borderRadius:BorderRadius.zero),side:const BorderSide(color:ink,width:2))),extensions:const <ThemeExtension<dynamic>>[]);}
}
